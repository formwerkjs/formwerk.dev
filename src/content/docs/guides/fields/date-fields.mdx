...
Use the `readonly` prop to make the date field read-only. Read-only date fields are not validated and are not submitted.
...
You can pass `min` and `max` props to set the minimum and maximum dates that can be entered.
...
You can use `@internationalized/date` to create calendar objects. In this example, we will use the `islamic-umalqura` calendar along with the `ar-EG` locale.
...
<template>
  <DateField label="Hijri Date" :calendar="calendar" locale="ar-EG" />
</template>
```
...
These are the properties that can be passed to the `useDateTimeField` composable.

<MdxTableAPI typeName="DateTimeFieldProps" />

#### Returns

These are the properties in the object returned by the `useDateTimeField` composable.

<MdxTableAPI fnName="useDateTimeField" />

### useDateTimeSegment

#### Props

These are the properties that can be passed to the `useDateTimeSegment` composable.
