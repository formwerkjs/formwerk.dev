...
ComboBoxes by default do not allow custom values. If you need to allow custom values, you need to pass the `onNewValue` prop/hander to `useComboBox`. The `onNewValue` prop is a function that must return the both the new value and the label to use for it.
